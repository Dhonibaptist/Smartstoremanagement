...
SELECT 'Store Owner', 'owner@store.com', '$2a$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', (SELECT id FROM roles WHERE name = 'owner'), '+919876543210'
WHERE NOT EXISTS (SELECT 1 FROM users WHERE email = 'owner@store.com');
...
SELECT name, contact_person, phone, email, gstin
FROM (VALUES
...
) AS v(name, contact_person, phone, email, gstin)
WHERE NOT EXISTS (SELECT 1 FROM suppliers);

-- Seed Products
INSERT INTO products (name, sku, barcode, category_id, supplier_id, purchase_price, selling_price, tax_percent, stock_quantity, reorder_level, unit)
SELECT name, sku, barcode, category_id, supplier_id, purchase_price, selling_price, tax_percent, stock_quantity, reorder_level, unit
FROM (VALUES
...
) AS v(name, sku, barcode, category_id, supplier_id, purchase_price, selling_price, tax_percent, stock_quantity, reorder_level, unit)
